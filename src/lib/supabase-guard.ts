 ...
 * CORREÇÃO: isReady=true SEMPRE que o cliente funciona, independente de sessão
 * A ausência de sessão é indicada por session=null e error='unauthenticated'
 */
export async function checkSupabaseReady(): Promise<SupabaseGuardResult> {
  try {
    ...
    if (sessionError) {
      ...
      return {
        isReady: true, // CORREÇÃO: Cliente funciona, mas há erro de sessão
        session: null,
        user: null,
    ...
    if (!session) {
      console.log('[SUPABASE_GUARD] ℹ️ Nenhuma sessão ativa (unauthenticated)');
      return {
        isReady: true, // CORREÇÃO: Cliente funciona, apenas não há sessão
        session: null,
        user: null,
        error: 'unauthenticated', // SEMÂNTICA CLARA
      };
    }
    ...
    if (!session.user) {
      ...
      return {
        isReady: true, // CORREÇÃO: Cliente funciona
        session: null,
        user: null,
    ...
    if (session.expires_at && session.expires_at < now) {
      ...
      return {
        isReady: true, // CORREÇÃO: Cliente funciona, token expirado
        session: null,
        user: null,
    ...
    // Tudo OK!
    console.log('[SUPABASE_GUARD] ✅ Usuário autenticado');
    return {
      isReady: true,
      session,
      user: session.user,
    ...
    return {
      isReady: false, // ERRO REAL: cliente não funciona
      session: null,
      user: null,
