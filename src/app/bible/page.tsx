...
// Forçar renderização no cliente (sem SSR/SSG)
...
function BiblePageContent() {
  ...
  // Ler parâmetros da URL ao carregar (para favoritos)
  useEffect(() => {
    if (!mounted) return;
    ...
    if (book && chapter) {
      ...
      // Se vier de um versículo favorito, destacá-lo
      if (verse) {
        setHighlightVerse(parseInt(verse));

        // Scroll suave até o versículo após um pequeno delay
        setTimeout(() => {
