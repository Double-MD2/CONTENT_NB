...
export default function BibleBooks({ onSelectBook, darkMode }: BibleBooksProps) {
  ...
  // Filtrar livros com base na busca
  const filteredOldTestament = useMemo(() => {
    if (!searchQuery.trim()) return oldTestament;

    const normalized = normalizeText(searchQuery);
    return oldTestament.filter(book =>
      normalizeText(book.name).includes(normalized) ||
      normalizeText(book.abbr).includes(normalized)
    );
  }, [searchQuery]);

  const filteredNewTestament = useMemo(() => {
    if (!searchQuery.trim()) return newTestament;
  ...
  const totalResults = filteredOldTestament.length + filteredNewTestament.length;
  const hasSearch = searchQuery.trim().length > 0;

  return (
    <div className="space-y-6">
      {/* Barra de Busca */}
      <div className={`sticky top-0 z-10 ${darkMode ? 'bg-gray-900' : 'bg-gradient-to-b from-blue-50 to-white'} pb-4`}>
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Buscar livro... (ex: João, Gênesis, Rm)"
            value={searchQuery}
            ...
            className={`w-full pl-12 pr-12 py-4 rounded-xl border-2 transition-all ${
              darkMode
                ...
                : 'bg-white border-gray-200 text-gray-800 placeholder-gray-400 focus:border-blue-400 focus:shadow-lg'
            } focus:outline-none`}
          />
          {hasSearch && (
            <button
      ...
      {filteredOldTestament.length > 0 && (
        ...
        <div className={`flex items-center gap-2 mb-4 pb-2 border-b-2 ${darkMode ? 'border-blue-500' : 'border-blue-400'}`}>
          ...
          <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            ({filteredOldTestament.length} {hasSearch ? 'de ' + oldTestament.length : ''} livros)
          </span>
        </div>
      ...
      {filteredNewTestament.length > 0 && (
        ...
        <div className={`flex items-center gap-2 mb-4 pb-2 border-b-2 ${darkMode ? 'border-green-500' : 'border-green-400'}`}>
          ...
          <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            ({filteredNewTestament.length} {hasSearch ? 'de ' + newTestament.length : ''} livros)
          </span>
        </div>
